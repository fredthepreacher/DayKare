...
function useHomeCandidate(
  ...
  const candidate = useMemo(
    () => ({ id, position: vector.clone(), valid: true, range, priority }),
    [id, vector, range, priority],
  );
  useEffect(() => registerInteractionCandidate(candidate), [candidate]);
  useFrame(() =>
    updateInteractionCandidate(id, { position: vector, valid: true }),
  );
  return null;
}
    ...
    <group>
      {HOME_SOLIDS.map((solid) => {
        ...
        return (
          ...
          >
            <boxGeometry
              args={[solid.maxX - solid.minX, height, solid.maxZ - solid.minZ]}
            />
...
function Slab({
  ...
  return (
    <mesh
      position={[(minX + maxX) / 2, y - 0.06, (minZ + maxZ) / 2]}
      receiveShadow
    >
  ...
  return (
    <group position={[x, y, z]}>
      <mesh position={[0, 0.62, 0]}>
        <cylinderGeometry args={[0.05, 0.09, 1.24, 10]} />
        <meshStandardMaterial color="#c8a05a" />
      </mesh>
      <mesh position={[0, 1.36, 0]}>
        <coneGeometry args={[0.3, 0.36, 14]} />
...
function ExitDoor() {
  ...
  useEffect(() => registerInteractionCandidate(candidate), [candidate]);
  useFrame(() =>
    updateInteractionCandidate("final-home-exit", {
      position: vector,
      valid: true,
    }),
  );
  return (
    <group position={[-1, 0, 8.02]}>
      <mesh position={[0, 1.15, 0]}>
        <boxGeometry args={[1.5, 2.3, 0.14]} />
        <meshStandardMaterial color="#8a5a44" />
      </mesh>
      <mesh position={[0.52, 1.12, -0.1]}>
        <sphereGeometry args={[0.08, 10, 8]} />
        <meshStandardMaterial color="#e8c15a" metalness={0.5} />
      </mesh>
      <Text
        position={[0, 1.95, -0.1]}
...
export function HomeInterior() {
  ...
  const ownsPingPong = useStorybookLaneStore((state) =>
    state.ownedItems.includes("ping-pong-table"),
  );
  ...
  return (
    <group>
      ...
      <pointLight
        position={[12, HOME_UPPER_Y + 2.4, 2]}
        intensity={0.45}
        distance={14}
      />
      <pointLight
        position={[-19, HOME_BASEMENT_Y + 2.2, 0]}
        intensity={0.5}
        distance={14}
      />
      ...
      <mesh position={[-8, 1.12, 1.42]} castShadow>
        <boxGeometry args={[2.25, 1.3, 0.09]} />
        <meshStandardMaterial color="#22262b" />
      </mesh>
      <mesh position={[-8, 1.12, 1.36]}>
      ...
      <mesh position={[-8, 0.44, 1.42]}>
        <boxGeometry args={[0.16, 0.32, 0.16]} />
        <meshStandardMaterial color="#3a4046" />
      </mesh>
      <mesh position={[-8, 0.68, 1.5]}>
        <boxGeometry args={[1.5, 0.11, 0.13]} />
        <meshStandardMaterial color="#3a4046" />
      </mesh>
      <mesh position={[-8.9, 0.7, 1.5]}>
        <boxGeometry args={[0.34, 0.09, 0.28]} />
        <meshStandardMaterial color={palette.accent} />
      </mesh>
      <mesh position={[-7, 0.42, 4]} castShadow>
        <boxGeometry args={[1.5, 0.44, 0.9]} />
        <meshStandardMaterial color="#a9713f" />
      </mesh>
      <Lamp x={-4.2} z={7} y={0} />
      <mesh position={[-9.85, 1.9, 4]}>
        <boxGeometry args={[0.06, 1, 1.5]} />
        <meshStandardMaterial color="#8ec6a4" />
      </mesh>
      <RoomLabel x={-7} z={3} y={0} text="LIVING ROOM" />

      {/* Kitchen + coffee maker */}
      <mesh position={[-9.2, 1.02, -7.2]} castShadow>
        <boxGeometry args={[0.34, 0.4, 0.3]} />
        <meshStandardMaterial color="#3e4750" />
      </mesh>
      <mesh position={[-6.4, 1.03, -7.2]} castShadow>
        <boxGeometry args={[0.62, 0.16, 0.44]} />
        <meshStandardMaterial color="#8fb6c9" />
      </mesh>
      ...
      {/* Bathroom 1 */}
      <mesh position={[0.2, 0.3, -7.2]} castShadow>
        <boxGeometry args={[0.5, 0.6, 0.62]} />
        <meshStandardMaterial color="#f4f8f9" />
      </mesh>
      <mesh position={[1.25, 1.5, -7.9]}>
        <boxGeometry args={[0.7, 0.62, 0.05]} />
        <meshStandardMaterial color="#cfe2e8" />
      </mesh>
      ...
      <mesh position={[11.45, HOME_UPPER_Y + 0.62, 7.5]} castShadow>
        <boxGeometry args={[2.2, 1.05, 0.16]} />
        <meshStandardMaterial color="#8d6244" />
      </mesh>
      {[-0.5, 0.5].map((offset) => (
        <mesh
          key={offset}
          position={[11.45 + offset, HOME_UPPER_Y + 0.7, 7.05]}
          castShadow
        >
      ...
      <mesh position={[11.45, HOME_UPPER_Y + 0.64, 5.9]} castShadow>
        <boxGeometry args={[2.05, 0.08, 1.5]} />
        <meshStandardMaterial color="#a3688c" />
      </mesh>
      ...
      {[2.0, 2.9].map((z) => (
        <mesh key={z} position={[16.36, HOME_UPPER_Y + 1.05, z]} castShadow>
          <boxGeometry args={[0.06, 1.9, 0.82]} />
          <meshStandardMaterial color="#c7a184" />
        </mesh>
      ))}
      <mesh position={[16.1, HOME_UPPER_Y + 1.75, 5.9]}>
        <boxGeometry args={[0.06, 0.85, 1.2]} />
        <meshStandardMaterial color="#f0a0b8" />
      </mesh>
      <RoomLabel x={13.2} z={3.6} y={HOME_UPPER_Y} text="PRIMARY BEDROOM" />
      <ClosetStation />

      {/* Upper: bathroom 2 + hallway */}
      <mesh position={[12.6, HOME_UPPER_Y + 0.3, -7.2]} castShadow>
        <boxGeometry args={[0.5, 0.6, 0.62]} />
        <meshStandardMaterial color="#f4f8f9" />
      </mesh>
      <RoomLabel x={13.4} z={-5.4} y={HOME_UPPER_Y} text="BATHROOM 2" />
      <Lamp x={7.4} z={5} y={HOME_UPPER_Y} />
      <mesh position={[6.2, HOME_UPPER_Y + 1.9, 5]}>
        <boxGeometry args={[0.06, 0.9, 1.3]} />
        <meshStandardMaterial color="#f0a0b8" />
      </mesh>
      <RoomLabel x={7.4} z={0} y={HOME_UPPER_Y} text="UPSTAIRS HALL" />

      {/* Ping pong is an owned furnishing, never a free/default table. */}
      {ownsPingPong ? <PingPongTable palette={palette} /> : null}
      ...
      {[
        ...
        <mesh
          key={`${bx}-${bz}`}
          position={[bx, HOME_BASEMENT_Y + 0.28, bz]}
          castShadow
        >
      ...
      <mesh position={[-20.2, HOME_BASEMENT_Y + 0.26, -4.2]} castShadow>
        <boxGeometry args={[0.9, 0.52, 0.7]} />
