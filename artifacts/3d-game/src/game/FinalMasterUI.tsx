...
export function FinalMasterOverlay() {
  ...
  const interact = () => {
    if (activeInteractable === 'final-miss-leslie') {
      state.recordTutorialEvent('talk-miss-leslie');
      const live = useFinalMasterStore.getState();
      if ((live.heistStatus === 'available' || live.heistStatus === 'complete') && live.startHeist()) {
        useFinalMasterStore.getState().recordHeistEvent('miss-leslie-intro');
        enqueueToast({ title: 'Miss Leslie has a plan', detail: 'Mia and Noah joined your team.' });
      } else enqueueToast({ title: 'Miss Leslie', detail: live.tutorialComplete ? 'Keep going—your team is right with you.' : 'Finish the tracked orientation steps first.' });
    } else if (activeInteractable?.startsWith('final-heist-')) {
      const event = activeInteractable.replace('final-heist-', '') as Parameters<typeof state.recordHeistEvent>[0];
      if (state.recordHeistEvent(event)) enqueueToast({ title: `${heist.title} updated`, detail: 'The real world objective was validated.', kind: 'success' });
    }
  };
  useEffect(() => {
  ...
  return <>
    ...
    {!state.tutorialComplete && chapter && <section className={`final-objective-card ${objectiveExpanded ? 'is-expanded' : ''}`} data-testid="final-tutorial"><button className="final-objective-toggle" onClick={() => setObjectiveExpanded((value) => !value)}><span><small>Orientation {state.tutorialChapter + 1} / 7</small><strong>{chapter.title}</strong></span><b>{objectiveExpanded ? '−' : '+'}</b></button>{objectiveExpanded && <><span>{chapter.objective}</span><ul>{chapter.steps.map((step) => <li key={step.id} className={state.tutorialCompletedSteps.includes(step.id) ? 'is-done' : ''}>{state.tutorialCompletedSteps.includes(step.id) ? '✓' : '○'} {step.label}</li>)}</ul><em>Reward: +{chapter.xp} XP{'reputation' in chapter ? ` · +${chapter.reputation} REP` : ''}</em>{!state.tutorialStarted ? <button onClick={() => state.startTutorial()}>Start Guided Practice</button> : <button onClick={() => enqueueToast({ title: chapter.title, detail: chapter.objective })}>Show Me</button>}</>}</section>}
    {state.tutorialComplete && <section className={`final-objective-card final-heist-card ${objectiveExpanded ? 'is-expanded' : ''}`} data-testid="final-heist-status"><button className="final-objective-toggle" onClick={() => setObjectiveExpanded((value) => !value)}><span><small><Users /> Miss Leslie’s Heist Board</small><strong>{state.heistStatus === 'active' ? heist.title : state.firstHeistComplete ? 'Daily Sticker Parade Replay' : 'A new plan is waiting'}</strong></span><b>{objectiveExpanded ? '−' : '+'}</b></button>{objectiveExpanded && <><span>{state.heistStatus === 'active' ? heist.objective : state.firstHeistComplete ? 'Complete one replay per in-game day for 5,000 RB, $1,000, and 250 XP.' : 'Physically find and interact with Miss Leslie near the Heist Board.'}</span>{state.heistStatus === 'active' && <ul>{heist.events.map((event) => <li key={event} className={state.heistCompletedEvents.includes(event) ? 'is-done' : ''}>{state.heistCompletedEvents.includes(event) ? '✓' : '○'} {event.replaceAll('-', ' ')}</li>)}</ul>}{state.firstHeistComplete && state.heistStatus !== 'active' && <button onClick={() => { const started = state.claimReplayReward(dayNumber); enqueueToast({ title: started ? 'Daily replay tracked' : 'Replay already completed today', detail: started ? 'Meet Miss Leslie to begin.' : undefined }); }}>Track daily replay</button>}</>}</section>}
