...
import { RALLY_WIN_RB, type RallyId } from "./rallyGame";
...
interface FinalMasterState {
  ...
  companionCommand:
    "follow" | "wait" | "goto" | "interact" | "regroup" | "finale";
  saveAvatar: (
    avatar: AvatarProfile,
    paid?: boolean,
  ) => "saved" | "insufficient";
...
function grantCoreReward(xp: number, cash: number, reputation = 0) {
  useGameStore.setState((state) => {
    ...
    const progression = {
      ...
      reputation: Math.min(
        MAX_REPUTATION,
        state.progression.reputation + reputation,
      ),
    };
    return {
      ...
      progression: {
        ...progression,
        routeUnlocks: getUnlockedRoutes(progression),
      },
    };
  });
}
...
export const useFinalMasterStore = create<FinalMasterState>()(
  persist(
    (set, get) => ({
      ...
      saveAvatar: (avatar, paid = false) => {
        if (paid) {
          ...
          useStorybookLaneStore.setState({
            ribbonBucks: rb - FULL_REDESIGN_PRICE,
          });
        }
        set({
          avatar: {
            ...avatar,
            name: avatar.name.trim().slice(0, 20) || "New Kid",
          },
      ...
      recordTutorialEvent: (event) => {
        ...
        if (
          ...
          !chapter ||
          state.tutorialCompletedSteps.includes(event)
        )
          return false;
        if (!chapter.steps.some((step) => step.id === event)) return false;
        const completed = [...state.tutorialCompletedSteps, event];
        const chapterDone = chapter.steps.every((step) =>
          completed.includes(step.id),
        );
        if (!chapterDone) {
          set({ tutorialCompletedSteps: completed });
          return true;
        }
        const alreadyRewarded = state.tutorialRewardedChapters.includes(
          chapter.id,
        );
        if (!alreadyRewarded) {
          grantCoreReward(
            chapter.xp,
            "cash" in chapter ? chapter.cash : 0,
            "reputation" in chapter ? chapter.reputation : 0,
          );
          useToastStore
            .getState()
            .enqueue({
              title: `${chapter.title} complete!`,
              detail: `+${chapter.xp} XP`,
              kind: "success",
            });
        }
        ...
        set({
          ...
          tutorialCompletedSteps: [],
          tutorialMovementDistance: 0,
          tutorialStarted: next < TUTORIAL_CHAPTERS.length,
          tutorialComplete: next >= TUTORIAL_CHAPTERS.length,
          tutorialRewardedChapters: alreadyRewarded
            ? state.tutorialRewardedChapters
            : [...state.tutorialRewardedChapters, chapter.id],
          activeAnimation: null,
        });
        return true;
      },
      recordTutorialMovement: (distance) => {
        ...
        if (
          !state.tutorialStarted ||
          TUTORIAL_CHAPTERS[state.tutorialChapter]?.id !== "welcome" ||
      ...
      skipTutorialForReturningPlayer: () =>
        set({
          avatarConfirmed: true,
          tutorialStarted: false,
          tutorialChapter: TUTORIAL_CHAPTERS.length,
          tutorialCompletedSteps: [],
          tutorialRewardedChapters: TUTORIAL_CHAPTERS.map(
            (chapter) => chapter.id,
          ),
      ...
      startHeist: () => {
        ...
        if (
          ...
          (state.firstHeistComplete && state.lastReplayDay === dayNumber)
        )
          return false;
        set({
          heistStatus: "active",
          ...
          heistCompletedEvents: [],
          companionCommand: "follow",
          activeAnimation: "anim_miss_leslie_board_intro",
          missLeslieHeistIntroduced: true,
          leoHeistApproachActive: false,
          leoHeistWaypointActive: false,
        });
        return true;
      ...
      recordHeistEvent: (event) => {
        ...
        if (
          !step ||
          !step.events.includes(event as never) ||
          state.heistCompletedEvents.includes(event)
        )
          return false;
        const completed = [...state.heistCompletedEvents, event];
        if (!step.events.every((required) => completed.includes(required))) {
          set({ heistCompletedEvents: completed });
          return true;
        }
        ...
        if (next >= HEIST_STEPS.length) {
          if (!state.firstHeistComplete) {
            ...
            set({
              heistStatus: "reward-choice",
              heistStep: HEIST_STEPS.length,
              heistCompletedEvents: completed,
              companionCommand: "finale",
              heistsCompleted: state.heistsCompleted + 1,
              successfulFinales: state.successfulFinales + 1,
            });
          } else {
            const dayNumber = useGameStore.getState().dayNumber;
            if (state.lastReplayDay !== dayNumber) {
              useStorybookLaneStore
                .getState()
                .grantRibbonBucks(REPLAY_HEIST_RB);
              grantCoreReward(FIRST_HEIST_XP, FIRST_HEIST_CASH);
            }
            set({
              heistStatus: "complete",
              heistStep: HEIST_STEPS.length,
              heistCompletedEvents: completed,
              companionCommand: "finale",
              lastReplayDay: dayNumber,
              heistsCompleted: state.heistsCompleted + 1,
              successfulFinales: state.successfulFinales + 1,
              totalHeistRbEarned: state.totalHeistRbEarned + REPLAY_HEIST_RB,
            });
            get().claimHeroOutfit();
          }
        } else
      ...
      chooseFirstReward: (choice) => {
        const state = get();
        if (state.heistStatus !== "reward-choice" || state.firstRewardChoice)
          return false;
        if (choice === "rb")
          useStorybookLaneStore.getState().grantRibbonBucks(FIRST_HEIST_RB);
        set({
          firstRewardChoice: choice,
          firstHeistComplete: true,
          heistStatus: "complete",
          homeVoucher: choice === "home",
          totalHeistRbEarned:
            state.totalHeistRbEarned + (choice === "rb" ? FIRST_HEIST_RB : 0),
        });
      ...
      claimReplayReward: (dayNumber) => {
        const state = get();
        if (!state.firstHeistComplete || state.lastReplayDay === dayNumber)
          return false;
        set({
          heistStatus: "active",
          heistStep: 0,
      ...
      completeRoutePlanner: (risk) => {
        ...
        const safeRisk = Math.max(
          0,
          Math.min(9, Math.floor(Number.isFinite(risk) ? risk : 9)),
        );
        const firstClear = !state.routePlannerComplete;
        set({
          routePlannerComplete: true,
          routePlannerBestRisk:
            state.routePlannerBestRisk === null
              ? safeRisk
              : Math.min(state.routePlannerBestRisk, safeRisk),
        });
        if (firstClear) {
          ...
          useToastStore
            .getState()
            .enqueue({
              title: "Low-risk route planned! +25 XP",
              detail:
                "Setup advantage saved. Major heist payouts are unchanged.",
              kind: "success",
            });
        }
        return firstClear;
      ...
      completeTimingGrid: (score) => {
        ...
        set({
          ...
          timingGridBestScore:
            state.timingGridBestScore === null
              ? safeScore
              : Math.max(state.timingGridBestScore, safeScore),
        });
        if (firstClear) {
          ...
          useToastStore
            .getState()
            .enqueue({
              title: "Timing read! +25 XP",
              detail: "Practice only \u2014 heist payouts are unchanged.",
              kind: "success",
            });
        }
        return firstClear;
      ...
      resolveHomeRewardRecovery: (choice) => {
        ...
        if (choice === "rb")
          useStorybookLaneStore.setState({
            ribbonBucks:
              useStorybookLaneStore.getState().ribbonBucks + FIRST_HEIST_RB,
          });
        return true;
      },
      cycleHomeTheme: () => {
      ...
      recordRallyResult: (id, bestRally, xp) => {
        const state = get();
        const safeBest = Math.max(
          0,
          Math.min(999, Math.floor(Number.isFinite(bestRally) ? bestRally : 0)),
        );
      ...
      awardRallyWin: (id, matchId) => {
        const state = get();
        const safeId = typeof matchId === "string" ? matchId.slice(0, 100) : "";
        if (
          !safeId ||
          (id !== "tennis" && id !== "ping-pong") ||
          state.rallyRewardedMatches.includes(safeId)
        )
          return false;
        set({
          rallyRewardedMatches: [
            ...state.rallyRewardedMatches.slice(-49),
            safeId,
          ],
          rallyWins: {
            ...state.rallyWins,
            [id]: (state.rallyWins[id] ?? 0) + 1,
          },
        });
        useStorybookLaneStore.getState().grantRibbonBucks(RALLY_WIN_RB);
        useToastStore
          .getState()
          .enqueue({
            title: `${id === "tennis" ? "Tennis" : "Ping-Pong"} win!`,
            detail: `+${RALLY_WIN_RB} Rascal Bucks`,
            kind: "success",
          });
        return true;
      },
      ...
      claimHeroOutfit: () => {
        const state = get();
        if (
          state.heroOutfitUnlocked ||
          !state.firstHeistComplete ||
          state.heistsCompleted < HERO_OUTFIT_HEISTS
        )
          return false;
      ...
      requestLeoHeistApproach: (absoluteMinute, eligible, leoStoryComplete) => {
        ...
        if (
          !eligible ||
          !leoStoryComplete ||
          state.missLeslieHeistIntroduced ||
          state.heistStatus === "active" ||
          state.heistStatus === "reward-choice" ||
          state.leoHeistHintCount >= 2 ||
          state.leoHeistApproachActive
        )
          return false;
        if (
          state.leoHeistHintCount === 1 &&
          absoluteMinute <
            (state.leoHeistNextHintMinute ?? Number.POSITIVE_INFINITY)
        )
          return false;
      ...
      buyStarterHome: () => {
        ...
        const rb = useStorybookLaneStore.getState().ribbonBucks;
        if (!state.homeVoucher && rb < STARTER_HOME_PRICE)
          return "insufficient";
        if (!state.homeVoucher)
          useStorybookLaneStore.setState({
            ribbonBucks: rb - STARTER_HOME_PRICE,
          });
      ...
      convertRbToGem: () => {
        ...
        useStorybookLaneStore.setState({
          ribbonBucks: rb - RASCAL_BUCKS_PER_GEM,
        });
    ...
    }),
    {
      ...
      partialize: (state) => ({
        avatar: state.avatar,
        avatarConfirmed: state.avatarConfirmed,
        tutorialStarted: state.tutorialStarted,
        tutorialChapter: state.tutorialChapter,
        tutorialCompletedSteps: state.tutorialCompletedSteps,
        tutorialRewardedChapters: state.tutorialRewardedChapters,
        tutorialMovementDistance: state.tutorialMovementDistance,
        tutorialComplete: state.tutorialComplete,
        heistStatus: state.heistStatus,
        heistStep: state.heistStep,
        heistCompletedEvents: state.heistCompletedEvents,
        firstHeistComplete: state.firstHeistComplete,
        firstRewardChoice: state.firstRewardChoice,
        lastReplayDay: state.lastReplayDay,
        heistsCompleted: state.heistsCompleted,
        successfulFinales: state.successfulFinales,
        totalHeistRbEarned: state.totalHeistRbEarned,
        routePlannerComplete: state.routePlannerComplete,
        routePlannerBestRisk: state.routePlannerBestRisk,
        timingGridComplete: state.timingGridComplete,
        timingGridBestScore: state.timingGridBestScore,
        homeRewardRecoveryPending: state.homeRewardRecoveryPending,
        homeThemeIndex: state.homeThemeIndex,
        garageThemeIndex: state.garageThemeIndex,
        rallyBest: state.rallyBest,
        rallyRewardedMatches: state.rallyRewardedMatches,
        rallyWins: state.rallyWins,
        neighborhoodDone: state.neighborhoodDone,
        heroOutfitUnlocked: state.heroOutfitUnlocked,
        leoHeistHintCount: state.leoHeistHintCount,
        leoHeistIntroCompleted: state.leoHeistIntroCompleted,
        leoHeistNextHintMinute: state.leoHeistNextHintMinute,
        leoHeistWaypointActive: state.leoHeistWaypointActive,
        missLeslieHeistIntroduced: state.missLeslieHeistIntroduced,
        ownedStarterHome: state.ownedStarterHome,
        homeVoucher: state.homeVoucher,
        gems: state.gems,
      }),
      merge: (persisted, current) => {
        const saved = (
          persisted && typeof persisted === "object" ? persisted : {}
        ) as Partial<FinalMasterState>;
        const chapter = Math.max(
          0,
          Math.min(
            TUTORIAL_CHAPTERS.length,
            Math.floor(saved.tutorialChapter ?? 0),
          ),
        );
        const rewarded = Array.isArray(saved.tutorialRewardedChapters)
          ? saved.tutorialRewardedChapters.filter((id) =>
              TUTORIAL_CHAPTERS.some((item) => item.id === id),
            )
        ...
        const firstReward =
          saved.firstRewardChoice === "rb" || saved.firstRewardChoice === "home"
            ? saved.firstRewardChoice
            : null;
        ...
        const voucher =
          saved.homeVoucher === true && firstReward !== "rb" && !ownsHome;
        ...
        return {
          ...
          homeThemeIndex: safeCount(
            saved.homeThemeIndex ?? 0,
            HOME_THEMES.length - 1,
          ),
          garageThemeIndex: safeCount(
            saved.garageThemeIndex ?? 0,
            GARAGE_THEMES.length - 1,
          ),
          rallyBest:
            saved.rallyBest && typeof saved.rallyBest === "object"
              ? Object.fromEntries(
                  Object.entries(saved.rallyBest)
                    .filter(([, value]) => Number.isFinite(Number(value)))
                    .map(([key, value]) => [key, safeCount(value, 999)]),
                )
              : {},
          rallyRewardedMatches: Array.isArray(saved.rallyRewardedMatches)
            ? [
                ...new Set(
                  saved.rallyRewardedMatches.filter(
                    (id): id is string =>
            ...
            : [],
          rallyWins:
            saved.rallyWins && typeof saved.rallyWins === "object"
              ? Object.fromEntries(
                  Object.entries(saved.rallyWins)
                    .filter(([id]) => id === "tennis" || id === "ping-pong")
                    .map(([id, value]) => [id, safeCount(value, 9999)]),
                )
              : {},
          ...
          neighborhoodDone: Array.isArray(saved.neighborhoodDone)
            ? saved.neighborhoodDone.filter((id) =>
                NEIGHBORHOOD_SPOT_IDS.includes(id),
              )
            : [],
          ...
          heroOutfitUnlocked:
            saved.firstHeistComplete === true &&
            safeCount(saved.heistsCompleted ?? 0, Number.MAX_SAFE_INTEGER) >=
              HERO_OUTFIT_HEISTS,
          rallyGameOpen: false,
          timingGridComplete: saved.timingGridComplete === true,
          timingGridBestScore:
            typeof gridScore === "number" && Number.isFinite(gridScore)
              ? Math.max(
                  0,
                  Math.min(TIMING_GRID_ROUNDS.length, Math.floor(gridScore)),
                )
              : null,
          tutorialChapter: chapter,
          tutorialComplete:
            saved.tutorialComplete === true ||
            chapter >= TUTORIAL_CHAPTERS.length,
          tutorialCompletedSteps: Array.isArray(saved.tutorialCompletedSteps)
            ? saved.tutorialCompletedSteps
            : [],
          tutorialRewardedChapters: rewarded,
          tutorialMovementDistance: Number.isFinite(
            Number(saved.tutorialMovementDistance),
          )
            ? Math.max(0, Number(saved.tutorialMovementDistance))
            : 0,
          heistCompletedEvents: Array.isArray(saved.heistCompletedEvents)
            ? saved.heistCompletedEvents
            : [],
          heistsCompleted: safeCount(
            saved.heistsCompleted ?? 0,
            Number.MAX_SAFE_INTEGER,
          ),
          successfulFinales: safeCount(
            saved.successfulFinales ?? 0,
            Number.MAX_SAFE_INTEGER,
          ),
          totalHeistRbEarned: safeCount(
            saved.totalHeistRbEarned ?? 0,
            Number.MAX_SAFE_INTEGER,
          ),
          routePlannerComplete: saved.routePlannerComplete === true,
          routePlannerBestRisk:
            typeof routeRisk === "number" && Number.isFinite(routeRisk)
              ? Math.max(0, Math.min(9, Math.floor(routeRisk)))
              : null,
          heistBoardOpen: false,
          leoHeistHintCount: safeCount(saved.leoHeistHintCount ?? 0, 2),
          leoHeistIntroCompleted: saved.leoHeistIntroCompleted === true,
          leoHeistNextHintMinute:
            typeof saved.leoHeistNextHintMinute === "number" &&
            Number.isFinite(saved.leoHeistNextHintMinute)
              ? Math.max(0, Math.floor(saved.leoHeistNextHintMinute))
              : null,
          leoHeistApproachActive: false,
          leoHeistWaypointActive:
            saved.leoHeistWaypointActive === true &&
            saved.missLeslieHeistIntroduced !== true,
          missLeslieHeistIntroduced:
            saved.missLeslieHeistIntroduced === true ||
            saved.heistStatus === "active" ||
            saved.firstHeistComplete === true,
        };
      },
    },
...
export function deleteDayKareSave() {
  ...
  [
    "daykare-save",
    "daykare-final-master",
    "daykare-storybook-lane",
    "daykare-monetization",
 ...
 */
registerHeistsCompletedReader(
  () => useFinalMasterStore.getState().heistsCompleted,
);
useGameStore.getState().syncDripOwnership();
