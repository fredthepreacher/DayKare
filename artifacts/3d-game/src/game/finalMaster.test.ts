...
assert.deepEqual([FIRST_HEIST_RB, FIRST_HEIST_CASH, FIRST_HEIST_XP], [9_000, 1_000, 250]);
...
assert.equal(useStorybookLaneStore.getState().ribbonBucks, FIRST_HEIST_RB, 'the first clear pays the authored Leslie amount');
...
assert.equal(useStorybookLaneStore.getState().ribbonBucks, FIRST_HEIST_RB + REPLAY_HEIST_RB);
assert.deepEqual([useFinalMasterStore.getState().heistsCompleted, useFinalMasterStore.getState().successfulFinales, useFinalMasterStore.getState().totalHeistRbEarned], [2, 2, FIRST_HEIST_RB + REPLAY_HEIST_RB], 'career heist stats persist separately from wallet balance');
...
assert.equal(useStorybookLaneStore.getState().ribbonBucks, FIRST_HEIST_RB + REPLAY_HEIST_RB, 'home voucher never spends RB');
