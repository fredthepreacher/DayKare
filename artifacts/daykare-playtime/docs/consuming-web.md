# Consuming DayKare Playtime Design System in web apps

Read `artifacts/daykare-playtime/docs/AGENTS.md` first. This guide covers
...
`artifacts/daykare-playtime/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/daykare-playtime/styles.css";
```
...
`node_modules/@workspace/daykare-playtime/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/daykare-playtime/components/ui/button";
import { cn } from "@workspace/daykare-playtime/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/daykare-playtime/hooks/use-toast";
```
...
`@workspace/daykare-playtime/components/ui/button`. Run the app's typecheck
