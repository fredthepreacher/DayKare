# Migrating Expo UI to DayKare Playtime Design System

Read `artifacts/daykare-playtime/docs/AGENTS.md` and
`artifacts/daykare-playtime/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/daykare-playtime/lib/native-theme` and
  `@workspace/daykare-playtime/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/daykare-playtime/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/daykare-playtime` and retains only
