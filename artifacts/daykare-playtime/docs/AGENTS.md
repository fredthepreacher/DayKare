# DayKare Playtime Design System design system
...
workspace package (`@workspace/daykare-playtime`): other artifacts depend
...
- `src/preview/patterns.tsx` — source-derived DayKare compositions for destination
  actions, choice rows, participant seats, content tone, and motion guidance.
- `docs/design-guidelines.md` — concise rules extracted from the DayKare game UI.
- `docs/references/` — retained source evidence and the primary DayKare playtime
  mark. Read its manifest before changing the palette, type, or signature patterns.
...
`@workspace/daykare-playtime/components/...`. Never use a `@/` alias inside
...
for `@workspace/daykare-playtime` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/daykare-playtime` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/daykare-playtime/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/daykare-playtime/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/daykare-playtime/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/daykare-playtime/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/daykare-playtime/docs/migrating-expo.md`
...
- Match exact token values. Do not invent colors, fonts, spacing, or radii in a
  consuming app.
- Keep the paper-and-cocoa foundation calm so the orange, teal, and pink activity
  colors remain meaningful instead of becoming decoration.
- Use the dark theme as a derived accessible counterpart. The current DayKare
  game UI supplied the light visual source; it did not supply matching dark
  versions of its custom panels.
